...
### クライアント: Swift/SwiftUI ネイティブ(第一候補)
...
### サーバー: 薄いプロキシ(Supabase Edge Functions または Hono on Cloudflare Workers)
